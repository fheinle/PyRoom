...
def start_autosave(autosave_time, edit_instance):
    """start the autosave timer"""
    timeout_id = gobject.timeout_add(60000, autosave_timeout, edit_instance)
...
def stop_autosave(edit_instance):
    ...
    for buf in edit_instance.buffers:
        if not buf.filename = edit_instance.UNNAME_FILENAME and \
           os.path.isfile(buf.filename + "~"):
...
def autosave_timeout(edit_instance):
    ...
    if edit_instance.autosave_elapsed >= edit_instance.autosave_time:
...
def autosave(edit_instance):
    ...
    for buf in edit_instance.buffers:
        if not buf.filename = edit_instance.UNNAMED_FILENAME:
            backup_file = open(edit_instance.filename + '~', 'w')
            try:
                backup_file.write(
            ...
            edit_instance.status.set_text(_('AutoSaving Buffer %(buf_id)d'))
